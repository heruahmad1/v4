...
echo -e "${BIWhite}      • ${BICyan}Operating System    ${NC}= $( cat /etc/os-release | grep -w PRETTY_NAME | sed 's/PRETTY_NAME//g' | sed 's/=//g' | sed 's/"//g') )"
echo -e "${BIWhite}      • ${BICyan}Server IP           ${NC}= ${ipsaya}"
echo -e "${BIWhite}      • ${BICyan}ISP-VPS             ${NC}= ${ISP}"
echo -e "${BIWhite}      • ${BICyan}City                ${NC}= ${CITY}"
echo -e "${BIWhite}      • ${BICyan}Current Domain      ${NC}= $( cat /etc/xray/domain )"
echo -e "${BIWhite}      • ${BICyan}NS Domain           ${NC}= $(cat /root/nsdomain) "
