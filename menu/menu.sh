...
echo -e "${BIWhite}      • ${GREEN}Operating System    ${NC}= $( cat /etc/os-release | grep -w PRETTY_NAME | sed 's/PRETTY_NAME//g' | sed 's/=//g' | sed 's/"//g') )"
...
echo -e "${BIWhite}      • ${GREEN}NS Domain           ${NC}= $(cat /root/nsdomain) "
echo -e "${BIWhite}      • ${GREEN}User Script Exp     ${NC}= ${BIYellow}$Name (${NC}${RED} $dayleft Days ${NC}${YELLOW})${NC}"
